...
#' 
#' @param m A \eqn{d\times d} lower (or upper) triangular matrix specifying
#' the regular vine structure.
#'
#' @return A \eqn{d\times d} upper triangular matrix if the input matrix is lower
...
#'  0, 0, 0, 4, 1,
#'  0, 0, 0, 0, 1)
#' LowerStrMtx <- matrix(M, 5, 5)
#' ##  Convert the lower triangular structure matrix to the upper triangular matrix.
#' revert(LowerStrMtx)
revert <- function (m) 
{
  if (length(dim(m)) == 2) {
